...
function sphere_srgd(
	...
	@inline output_functions.push!(output, 0.0, 0.0, f(Y, data), Hermitian(Y * Y'))
	...
	@inbounds for (t, i) in enumerate(rand_indices)
		...
		if t % period == 0 || t == max_iter
			...
			fval = f(Y, data)
			output_functions.push!(output, float(t) / n_data, time, fval, Hermitian(Y * Y'))
		end
